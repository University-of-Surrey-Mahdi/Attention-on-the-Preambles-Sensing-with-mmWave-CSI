...
| `sub_modules` | `/pre_angle.py` | Code | Pre-process | Angular domain analysis. Estimated channels for each Tx/Rx antenna (number of antennas is 4 for both Tx/Rx) are transfered to estimated channels for each angle $(\hat{\boldsymbol{H}} \Rightarrow \hat{\boldsymbol{H}}_{\textrm{vel}})$.|
